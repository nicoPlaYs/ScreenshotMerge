...
QString nextScreenshotName(){
	...
	QString path = settings.value("SettingsWindow/defaultDirectory",
	...
	for(int i = 2; i < 99999 && (nextScreenshotJPG.exists() || nextScreenshotPNG.exists()); i++){
		nextScreenshotJPG.setFileName(path + QString("\\Screenshot")
									  + QString::number(i).rightJustified(5, '0', false) + ".jpg");
		nextScreenshotPNG.setFileName(path + QString("\\Screenshot")
									  + QString::number(i).rightJustified(5, '0', false) + ".png");
	}
...
void saveImage(QPixmap image){
	...
	if(!settings.value("SettingsWindow/alwaysUseDefaultDirectory", false).toBool()){
		...
		path = QFileDialog::getSaveFileName(0, QObject::tr("Save as..."),
											nextScreenshotName(), "JPG (*.jpg);;PNG (*.png)");
	}
	else{
	...
	if(path != 0){
		...
		if(settings.value("SettingsWindow/alwaysUseDefaultDirectory", false).toBool()){
			QMessageBox::information(0, QObject::tr("Image successfully saved"),
									 QObject::tr("Your image has been saved in your default directory !"));
		}
	}
}
