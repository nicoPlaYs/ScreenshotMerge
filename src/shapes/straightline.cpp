...
bool StraightLine::isHitted(QRect hitbox){
	...
	if(static_cast<int>(straightLine.x1()) != static_cast<int>(straightLine.x2())){
		double a;
		double b;
		...
		while(x <= xMax){
			...
			if(hitbox.contains(QPoint(static_cast<int>(x), static_cast<int>(y)))){
				return true;
			}
	...
	// and if it's the case, with all the possibles values of y
		...
		while(y <= yMax){
			if(hitbox.contains(QPoint(static_cast<int>(straightLine.x1()), static_cast<int>(y)))){
				return true;
			}
