...
SettingsWindow::SettingsWindow() : QDialog(){
	...
	QObject::connect(spinBoxImageQuality, SIGNAL(valueChanged(int)),
					 sliderImageQuality, SLOT(setValue(int)));
	QObject::connect(sliderImageQuality, SIGNAL(valueChanged(int)),
					 spinBoxImageQuality, SLOT(setValue(int)));
	layout->addWidget(spinBoxImageQuality, 0, 2, 1, 1);


	// Default directory
	labelDefaultDirectory = new QLabel(tr("Default directory : "), this);
	lineEditDefaultDirectory = new QLineEdit(this->settings->value("SettingsWindow/defaultDirectory",
																   QStandardPaths::writableLocation(QStandardPaths::PicturesLocation)).toString(),
											 this);
	lineEditDefaultDirectory->setDisabled(true);
	changeDefaultDirectory = new QPushButton(tr("..."), this);
	QObject::connect(changeDefaultDirectory, SIGNAL(clicked()),
					 this, SLOT(openSelectDefaultDirectory()));
	...
	checkBoxAlwaysUseDefaultDirectory->setChecked(this->settings->value("SettingsWindow/alwaysUseDefaultDirectory",
																		false).toBool());
	...
	QObject::connect(checkBoxStartWithWindows, SIGNAL(toggled(bool)),
					 checkBoxMinimizeTray, SLOT(setChecked(bool)));
	QObject::connect(checkBoxStartWithWindows, SIGNAL(toggled(bool)),
					 checkBoxMinimizeTray, SLOT(setDisabled(bool)));
	...
	QObject::connect(buttonValidateChanges, SIGNAL(clicked()),
					 this, SLOT(validateChanges()));
	layoutValidateOrCancel->addWidget(buttonValidateChanges);

	buttonCancelChanges = new QPushButton(tr("Cancel"), validateOrCancel);
	QObject::connect(buttonCancelChanges, SIGNAL(clicked()),
					 this, SLOT(close()));
...
void SettingsWindow::validateChanges(){
	...
	settings->setValue("SettingsWindow/imageQuality",
					   sliderImageQuality->value());
	settings->setValue("SettingsWindow/defaultDirectory",
					   lineEditDefaultDirectory->text());
	settings->setValue("SettingsWindow/alwaysUseDefaultDirectory",
					   checkBoxAlwaysUseDefaultDirectory->isChecked());
	settings->setValue("SettingsWindow/minimizeTray",
					   checkBoxMinimizeTray->isChecked());
