...
MainWindow::MainWindow() : QMainWindow(){
	...
	actionTakeScreenshot = toolBar->addAction(QIcon("://images/toolbar/takescreenshot.ico"),
											  tr("Take a screenshot (Print Screen)"));
	QObject::connect(actionTakeScreenshot, SIGNAL(triggered()),
	...
	actionSaveMerged = toolBar->addAction(QIcon("://images/toolbar/save.ico"),
										  tr("Merge and save it (Ctrl+S)"));
	actionSaveMerged->setShortcut(QKeySequence::Save);
	QObject::connect(actionSaveMerged, SIGNAL(triggered()),
					 this, SLOT(saveMergedScreenshots()));

	actionCopyIntoClipboard = toolBar->addAction(QIcon("://images/editwindow/copytoclipboard.ico"),
	...
	QObject::connect(actionCopyIntoClipboard, SIGNAL(triggered()),
					 this, SLOT(copyIntoClipboardMergedScreenshots()));

	actionUploadMergedScreenshots = toolBar->addAction(QIcon("://images/toolbar/upload.ico"),
													   tr("Merge and upload it (Ctrl+U)"));
	actionUploadMergedScreenshots->setShortcut(QKeySequence("Ctrl+U"));
	QObject::connect(actionUploadMergedScreenshots, SIGNAL(triggered()),
					 this, SLOT(uploadMergedScreenshots()));
	...
	QObject::connect(actionDown, SIGNAL(triggered()),
					 this, SLOT(downImage()));
	...
	QObject::connect(listWidgetImage, SIGNAL(itemDoubleClicked(QListWidgetItem*)),
					 this, SLOT(openEditWindowOldScreenshot(QListWidgetItem*)));
	QObject::connect(actionClearList, SIGNAL(triggered()),
					 listWidgetImage, SLOT(clear()));
	...
	QObject::connect(trayIcon, SIGNAL(activated(QSystemTrayIcon::ActivationReason)),
					 this, SLOT(activationTrayIcon(QSystemTrayIcon::ActivationReason)));
...
void MainWindow::takeScreenshot(){
	...
	QObject::connect(crop, SIGNAL(cropOver(Screenshot*)),
					 this, SLOT(openEditWindowNewScreenshot(Screenshot*)));
}
...
void MainWindow::openEditWindowNewScreenshot(Screenshot* screenshot){
	...
	EditWindow* editWindow = new EditWindow(screenshot, comboBoxImageHost->currentData().value<ImageHost>(),
											listWidgetImage);
	if(this->isVisible()){
		QObject::connect(editWindow, SIGNAL(editOver(QString)),
						 this, SLOT(restore()));
	}
	QObject::connect(editWindow, SIGNAL(retakeSignal()),
					 this, SLOT(takeScreenshot()));
	QObject::connect(editWindow, SIGNAL(editOver(QString)),
					 comboBoxImageHost, SLOT(setCurrentText(QString)));
}
...
void MainWindow::uploadMergedScreenshots(){
	...
	if(mergedScreenshots != 0){
		...
		uploadImage(*mergedScreenshots, comboBoxImageHost->currentData().value<ImageHost>(),
...
void MainWindow::openEditWindowOldScreenshot(QListWidgetItem* screenshotClicked){
	// Create a window to edit the screenshot
	EditWindow* editWindow = new EditWindow(static_cast<Screenshot*>(screenshotClicked),
											comboBoxImageHost->currentData().value<ImageHost>());
	QObject::connect(editWindow, SIGNAL(editOver(QString)),
					 this, SLOT(restore()));
	QObject::connect(editWindow, SIGNAL(editOver(QString)),
					 comboBoxImageHost, SLOT(setCurrentText(QString)));
