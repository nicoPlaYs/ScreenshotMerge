...
UploadWindow::UploadWindow(QNetworkReply* reply, ImageHost host) : QDialog()
{
	this->host = host;
	this->reply = reply;
	QObject::connect(this->reply, SIGNAL(uploadProgress(qint64,qint64)),
					 this, SLOT(updateUploadProgress(qint64,qint64)));
	QObject::connect(this->reply, SIGNAL(finished()),
	...
	QObject::connect(copyLinkInClipboardButton, SIGNAL(pressed()),
					 this, SLOT(copyLinkInClipboard()));
...
void UploadWindow::updateUploadProgress(qint64 bytesSent, qint64 bytesTotal)
{
	if (bytesTotal != -1 && bytesSent != bytesTotal){
		uploadProgressBar->setRange(0, static_cast<int>(bytesTotal));
		uploadProgressBar->setValue(static_cast<int>(bytesSent));
	}
}
